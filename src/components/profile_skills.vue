<template>
  <div class="sections" v-if="!this.loading">
    <div class="section" v-for="section in sections" v-bind:key="section.header.title">
      <div v-bind:class="`header ${section.header.name}`">
        <span class="header_text">{{ section.header.title }}</span>
      </div>
      <div class="line"></div>
      <div class="items">
        <div v-for="item in section.items" v-bind:class="`item ${item.name} ${item.type} ${item.visible ? '' : 'invisible'}`" v-bind:key="item.name">
          <div class="left" v-if="item.type === 'both' || item.type === 'left'">
            <img v-bind:src="item.left" alt=""></img>
          </div>
          <div class="description">
            <h1>{{ item.title }}
              <i class="ion-android-open" v-if="item.url" v-on:click="openLink(item.url)"></i>
            </h1>
            <h5 v-if="item.subtitle">{{ item.subtitle }}</h5>
            <div class="des_container">
              <p>{{ item.description }}</p>
            </div>
            <h3 v-if="item.type !== 'center' && item.type !== 'shared' && item.skills">Skills Gained</h3>
            <div class="skills" v-if="item.type !== 'center' && item.type !== 'shared'">
              <div v-for="skill in item.skills" v-bind:class="`skill ${skill.type}`" v-bind:key="skill.name">
                <p>{{ skill.name }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="description" v-if="item.type === 'center' || item.type === 'shared'">
          ...
          <div class="right" v-if="item.type === 'both' || item.type === 'right'">
      ...
      .items {
        ...
        .item {
          ...
          img {
            width: 100%;
            max-width: 1200px;
            border-radius: 5px; // box-shadow: 0 14px 28px rgba(0,0,0,0.25), 0 10px 10px rgba(0,0,0,0.22);
          }
          .left {
            flex: 2;
            z-index: 99;
